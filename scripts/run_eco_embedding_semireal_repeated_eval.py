#!/usr/bin/env python3
"""Evaluación repetida de la ruta embedding semi-real para E.C.O.
...
from scripts.run_eco_embedding_semireal import build_embedding_report  # noqa: E402
...
MODEL_ORDER = ["baseline_v1", "baseline_v3", "embedding_semireal"]
MODEL_ROLES = {
    "baseline_v1": "control_explicable",
    "baseline_v3": "candidato_pre_embeddings",
    "embedding_semireal": "transicion_vectorial_controlada",
}
...
def run_once(records: Sequence[LabeledSequence], seed: int, test_ratio: float, embedding_k: int, dimensions: int) -> Dict[str, object]:
    ...
    return {
        ...
        "delta_macro_f1_vs_v3": round(scores["embedding_semireal"]["test_macro_f1"] - scores["baseline_v3"]["test_macro_f1"], 4),
        "delta_macro_f1_vs_v1": round(scores["embedding_semireal"]["test_macro_f1"] - scores["baseline_v1"]["test_macro_f1"], 4),
    }
...
def interpretation(summary: Dict[str, object]) -> str:
    ...
    if status == "empate_tecnico_con_v3":
        return (
            "El embedding semi-real queda en empate técnico frente a baseline_v3. "
    ...
    if status == "semireal_prometedor":
        return (
            "El semi-real muestra una señal positiva frente a v3, pero debe leerse solo como validación arquitectónica; "
            "todavía no es DNABERT ni embedding profundo real."
        )
    if status == "v3_sigue_como_referencia":
        return "baseline_v3 sigue siendo la referencia más fuerte; el semi-real valida contrato vectorial, no superioridad de modelo."
    return "La comparación deja una señal mixta; conviene mantener v1/v3 como controles y usar el semi-real solo como puente hacia embeddings reales."
...
def build_payload(records: Sequence[LabeledSequence], repeats: int, test_ratio: float, base_seed: int, embedding_k: int, dimensions: int) -> Dict[str, object]:
    ...
    return {
        "pipeline": "E.C.O. embedding semi-real repeated evaluation",
        "dataset_size": len(records),
        "repeats": repeats,
        "test_ratio": test_ratio,
        "base_seed": base_seed,
        "embedding": {
            "type": "kmer_frequency_semireal",
            "k": embedding_k,
        ...
        "limits": [
            ...
            "El embedding semi-real usa frecuencias k-mer; no es DNABERT ni embedding profundo real.",
...
def build_markdown(payload: Dict[str, object], input_path: Path) -> str:
    ...
    outcome_rows = [
        [
            "embedding_semireal vs baseline_v3",
        ...
        [
            "embedding_semireal vs baseline_v1",
    ...
    run_rows = [
        [
            ...
            run["models"]["embedding_semireal"]["test_macro_f1"],
    ...
    lines = [
        "# E.C.O. - Evaluación repetida de embedding semi-real",
        "",
        "## Propósito",
        "",
        "Este informe repite la ruta `embedding_semireal` con distintos splits estratificados y la compara contra `baseline_v1` y `baseline_v3`.",
        "",
        "## Configuración",
        "",
        *table(
...
<body>
  <header><h1>E.C.O. - Evaluación repetida de embedding semi-real</h1><p>Contrato vectorial repetido contra v1 y v3.</p></header>
  <main>
    ...
    <section class='section'><h2>Resultado del semi-real</h2>{html_table(['Comparación', 'Delta prom.', 'Delta std', 'Gana', 'Empata', 'Pierde'], outcome_rows)}</section>
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Evaluación repetida de embedding semi-real E.C.O.")
...
def main() -> int:
    ...
    summary = payload["summary"]
    print("E.C.O. EMBEDDING SEMI-REAL REPEATED EVALUATION")
    print("================================================")
    ...
    print("Estado: OK, evaluación repetida de embedding semi-real generada.")
    return 0
